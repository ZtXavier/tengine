...
ngx_int_t ngx_http_test_traffic_status_node_generate_key(ngx_pool_t *pool,
    ngx_str_t *buf, ngx_str_t *dst, unsigned type);
ngx_int_t ngx_http_test_traffic_status_node_position_key(ngx_str_t *buf,
    size_t pos);

ngx_rbtree_node_t *ngx_http_test_traffic_status_node_lookup(
    ngx_rbtree_t *rbtree, ngx_str_t *key, uint32_t hash);
void ngx_http_test_traffic_status_node_zero(
    ngx_http_test_traffic_status_node_t *vtsn);
void ngx_http_test_traffic_status_node_init(ngx_http_request_t *r,
    ngx_http_test_traffic_status_node_t *vtsn);
...
void ngx_http_test_traffic_status_find_name(ngx_http_request_t *r,
    ngx_str_t *buf);
ngx_rbtree_node_t *ngx_http_test_traffic_status_find_node(ngx_http_request_t *r,
    ngx_str_t *key, unsigned type, uint32_t key_hash);

ngx_rbtree_node_t *ngx_http_test_traffic_status_find_lru(ngx_http_request_t *r);
ngx_rbtree_node_t *ngx_http_test_traffic_status_find_lru_node(ngx_http_request_t *r,
    ngx_rbtree_node_t *a, ngx_rbtree_node_t *b);
ngx_rbtree_node_t *ngx_http_test_traffic_status_find_lru_node_cmp(ngx_http_request_t *r,
    ngx_rbtree_node_t *a, ngx_rbtree_node_t *b);

ngx_int_t ngx_http_test_traffic_status_node_member_cmp(ngx_str_t *member, const char *name);
ngx_atomic_uint_t ngx_http_test_traffic_status_node_member(
    ngx_http_test_traffic_status_node_t *vtsn,
    ngx_str_t *member);
