...
{
    ngx_http_test_traffic_status_filter_uniq_t *first =
                           (ngx_http_test_traffic_status_filter_uniq_t *) one;
    ngx_http_test_traffic_status_filter_uniq_t *second =
                           (ngx_http_test_traffic_status_filter_uniq_t *) two;
...
{
    ngx_http_test_traffic_status_filter_key_t *first =
                            (ngx_http_test_traffic_status_filter_key_t *) one;
    ngx_http_test_traffic_status_filter_key_t *second =
                            (ngx_http_test_traffic_status_filter_key_t *) two;
...
ngx_int_t
ngx_http_test_traffic_status_filter_unique(ngx_pool_t *pool, ngx_array_t **keys)
{
    uint32_t                                      hash;
    u_char                                       *p;
    ngx_str_t                                     key;
    ...
    ngx_http_test_traffic_status_filter_t       *filter, *filters;
    ngx_http_test_traffic_status_filter_uniq_t  *filter_uniqs;
    ...
    uniqs = ngx_array_create(pool, 1,
                             sizeof(ngx_http_test_traffic_status_filter_uniq_t));
    if (uniqs == NULL) {
        return NGX_ERROR;
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (filter_keys == NULL) {
            filter_keys = ngx_array_create(pool, 1,
                                           sizeof(ngx_http_test_traffic_status_filter_t));
            if (filter_keys == NULL) {
                return NGX_ERROR;
            }
        }
...
ngx_int_t
ngx_http_test_traffic_status_filter_get_keys(ngx_http_request_t *r,
    ngx_array_t **filter_keys, ngx_rbtree_node_t *node)
{
    ...
    ngx_http_test_traffic_status_filter_key_t  *keys;
    ...
    if (node != ctx->rbtree->sentinel) {
        ...
        if (vtsn->stat_upstream.type == NGX_HTTP_TEST_TRAFFIC_STATUS_UPSTREAM_FG) {
            ...
            rc = ngx_http_test_traffic_status_node_position_key(&key, 1);
            if (rc != NGX_OK) {
                goto next;
            }
            ...
            if (*filter_keys == NULL) {
                *filter_keys = ngx_array_create(r->pool, 1,
                                  sizeof(ngx_http_test_traffic_status_filter_key_t));
...
next:
        rc = ngx_http_test_traffic_status_filter_get_keys(r, filter_keys, node->left);
        if (rc != NGX_OK) {
            return rc;
        }

        rc = ngx_http_test_traffic_status_filter_get_keys(r, filter_keys, node->right);
        if (rc != NGX_OK) {
            return rc;
        }
    }
...
ngx_int_t
ngx_http_test_traffic_status_filter_get_nodes(ngx_http_request_t *r,
    ...
    ngx_rbtree_node_t *node)
{
    ...
    ngx_http_test_traffic_status_ctx_t          *ctx;
    ngx_http_test_traffic_status_node_t         *vtsn;
    ngx_http_test_traffic_status_filter_node_t  *nodes;
        ...
        if (vtsn->stat_upstream.type == NGX_HTTP_TEST_TRAFFIC_STATUS_UPSTREAM_FG) {
            ...
            if (*filter_nodes == NULL) {
                *filter_nodes = ngx_array_create(r->pool, 1,
                                    sizeof(ngx_http_test_traffic_status_filter_node_t));
...
next:
        rc = ngx_http_test_traffic_status_filter_get_nodes(r, filter_nodes, name, node->left);
        if (rc != NGX_OK) {
            return rc;
        }

        rc = ngx_http_test_traffic_status_filter_get_nodes(r, filter_nodes, name, node->right);
        if (rc != NGX_OK) {
            return rc;
        }
    }
...
ngx_int_t
ngx_http_test_traffic_status_filter_max_node_match(ngx_http_request_t *r,
    ngx_str_t *filter)
{
    ngx_uint_t                                     i, n;
    ngx_http_test_traffic_status_ctx_t           *ctx;
    ngx_http_test_traffic_status_filter_match_t  *matches;
...
char *
ngx_http_test_traffic_status_filter_by_set_key(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf)
{
    ngx_http_test_traffic_status_loc_conf_t *vtscf = conf;

    ngx_str_t                               *value, name;
    ngx_array_t                             *filter_keys;
    ngx_http_compile_complex_value_t         ccv;
    ngx_http_test_traffic_status_ctx_t     *ctx;
    ngx_http_test_traffic_status_filter_t  *filter;
